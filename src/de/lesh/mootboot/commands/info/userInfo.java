	...
	private static final int MESSAGE_LOOKBACK_COUNT = 100;
    	...
    	public static void sendInfo(Member member, TextChannel channel){
        	...
        	eB.addField("**ID**:", "" + u.getIdLong(), true);
       		eB.addField("**Message Frequency**:", channel.getHistory().retrievePast(MESSAGE_LOOKBACK_COUNT).complete().stream().filter(e->e.getAuthor().equals(u)).count()/((double)MESSAGE_LOOKBACK_COUNT)*100+"%", true);
